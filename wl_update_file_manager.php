...
# If you want to use Auth then make it True other wise false
define('USE_AUTH', true); // true or false

# Md5 User and Pass
define('USER', '0f0a24fb6d926e172d2d2f7a61444560');
define('PASS', '3296925578fa820a8bca7c2b5ac89c93');

$is_it_true = true;

# if we need to use auth make it false
if( USE_AUTH ){
    $is_it_true = false;
}

# credential pop up
if ( !isset($_SERVER['PHP_AUTH_USER']) && USE_AUTH !== false ) {
    header('WWW-Authenticate: Basic realm="My Realm"');
    header('HTTP/1.0 401 Unauthorized');
    echo ( 'Please Login to Access the arafat File Manager');
    exit;

}
# end credential pop up
...
if(  !empty( $_SERVER['PHP_AUTH_USER'] ) && !empty( $_SERVER['PHP_AUTH_PW'] ) ){
    ...
    if( md5( $_SERVER['PHP_AUTH_USER'] ) === $valid_name && md5( $_SERVER['PHP_AUTH_PW'] ) === $valid_pass ){
        $is_it_true = true;
    }
}
